...
std::vector<int> SelectMultiplexer::waitForEvents()
{
    ...
    int ready_fds = select(m_maxfd + 1,
                           &tmp_readfds,
                           &tmp_writefds,
                           &tmp_errorfds,
                           nullptr);
    ...
    {
        if (FD_ISSET(fd, &tmp_readfds) || FD_ISSET(fd, &tmp_writefds) ||
            FD_ISSET(fd, &tmp_errorfds))
        {
...
{
    auto it = std::remove_if(m_pollfds.begin(),
                             m_pollfds.end(),
...
EpollMultiplexer::EpollMultiplexer()
{
    ...
    if (-1 == m_epollfd)
    {
        LUNAR_LOG_ERROR(g_logger) << "Failed to create epoll instance";
    }
}
...
void EpollMultiplexer::removeFileDescriptor(int fd)
{
    ...
    {
        LUNAR_LOG_ERROR(g_logger)
            << "Failed to remove file descriptor from epoll instance";
    }
}
...
std::vector<int> EpollMultiplexer::waitForEvents()
{
    ...
    {
        LUNAR_LOG_ERROR(g_logger)
            << "Failed to wait for events using epoll instance";
    }
