...
void RpcProvider::notifyService(Service *service)
{
    ...
    LUNAR_LOG_INFO(g_rpclogger) << "方法名称 " << serviceName;
    // 获取服务对象的方法数量
    int methondCnt = serviceDesc->method_count();
    LUNAR_LOG_INFO(g_rpclogger) << "方法数量 " << methondCnt;
    ServiceInfo info;
    ...
    LUNAR_LOG_INFO(g_rpclogger)
        << m_serviceMap.size() << " "
        << m_serviceMap.find(serviceName)
               ->second.methodMap.find(serviceDesc->method(0)->name())
               ->first;

    LUNAR_LOG_INFO(g_rpclogger)
        << "publish servic: [" << serviceName.c_str() << "][" << methondCnt
        << "][" << serviceDesc->method(0)->name();
}
...
void RpcProvider::run()
{
    ...
    LUNAR_LOG_INFO(g_rpclogger) << "开始绑定回调";
    ...
    LUNAR_LOG_INFO(g_rpclogger) << "绑定回调结束";

    // ZkClientRPC::ptr zkCli = std::make_shared<ZkClientRPC>();
    m_zkClient.start();
    LUNAR_LOG_INFO(g_rpclogger) << "zkclient 开始";
    ...
    LUNAR_LOG_INFO(g_rpclogger)
        << "RpcProvider start service at 127.0.0.1:8000";
...
void RpcProvider::onMessage(const Socket::ptr &conn)
{
    ...
    LUNAR_LOG_INFO(g_rpclogger) << message;
    ...
    {
        LUNAR_LOG_ERROR(g_rpclogger)
            << "failed to parse rpc header: " << content.c_str();
        return;
    }
    ...
    LUNAR_LOG_DEBUG(g_rpclogger)
        << "receive rpc header: " << headerSize << " " << content.c_str() << " "
        << serviceName.c_str() << " " << methodName.c_str() << " " << argsSize
        << " " << argsStr.c_str();

    // 获取 service 对象和 method 对象
    LUNAR_LOG_ERROR(g_rpclogger) << m_serviceMap.size();
    auto iter = m_serviceMap.find(serviceName);
    if (iter == m_serviceMap.end())
    {
        LUNAR_LOG_ERROR(g_rpclogger) << serviceName.c_str() << " is not exist";
        return;
    }
    ...
    {
        LUNAR_LOG_ERROR(g_rpclogger) << serviceName.c_str() << ":"
                                     << methodName.c_str() << " is not exist";
        return;
    }
    ...
    {
        LUNAR_LOG_ERROR(g_rpclogger)
            << "request parse error: " << argsStr.c_str();
        return;
    }

    // 根据远端RPC请求 调用当前RPC节点上发布的方法
    Message *response = service->GetResponsePrototype(methodDesc).New();
    Closure *done = NewCallback<RpcProvider, const Socket::ptr &, Message *>(
        this,
...
{
    LUNAR_LOG_INFO(g_rpclogger) << "send rpc response";

    std::string responseStr;
    if (response->SerializeToString(&responseStr))
    {
        LUNAR_LOG_ERROR(g_rpclogger)
            << responseStr << " " << responseStr.size();
    ...
    {
        LUNAR_LOG_ERROR(g_rpclogger) << "failed to serial string";
    }
    conn->close();
}
