#ifndef __LUNAR_RPC_RPC_CLIENT_H__
#define __LUNAR_RPC_RPC_CLIENT_H__
...
public:
    ...
    void setConnectionCallback(std::function<void(const Socket::ptr &)> cb)
    {
        m_connectionCallback = std::move(cb);
    }
    void setMessageCallback(std::function<void(const Socket::ptr &)> cb)
    {
        m_messageCallback = std::move(cb);
    }
    void setWriteCompleteCallback(std::function<void(const Socket::ptr &)> cb)
    {
        m_writeCopmleteCallback = std::move(cb);
    }
