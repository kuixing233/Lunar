...
static lunar::ConfigVar<
    std::map<std::string, std::map<std::string, std::string>>>::ptr
    g_mysql_dbs = lunar::Config::Lookup(
        "mysql.dbs",
                         ...
                         const int &timeout)
{
    ...
    if (mysql == nullptr)
    {
        LUNAR_LOG_ERROR(g_logger) << "mysql_init error";
        return nullptr;
    }
    ...
    int port = lunar::GetParamValue(params, "port", 0);
    std::string host = lunar::GetParamValue<std::string>(params, "host");
    std::string user = lunar::GetParamValue<std::string>(params, "user");
    std::string passwd = lunar::GetParamValue<std::string>(params, "passwd");
    std::string dbname = lunar::GetParamValue<std::string>(params, "dbname");
...
bool MySQL::connect()
{
    ...
    m_hasError = false;
    m_poolSize = lunar::GetParamValue(m_params, "pool", 5);
    // 指定当 m 销毁时使用 mysql_close 进行销毁
    m_mysql.reset(m, mysql_close);
    return true;
}

lunar::IStmt::ptr MySQL::prepare(const std::string &sql)
{
...
{
    m_cmd = lunar::StringUtil::Formatv(format, ap);
...
{
    return MySQL::ptr(this, lunar::nop<MySQL>);
}
...
static MYSQL_RES *my_mysql_query(MYSQL *mysql, const char *sql)
{
    if (mysql == nullptr)
    {
        LUNAR_LOG_ERROR(g_logger) << "mysql_query mysql is null";
        return nullptr;
    }

    if (sql == nullptr)
    {
        LUNAR_LOG_ERROR(g_logger) << "mysql_query sql is null";
        return nullptr;
    }
...
int MySQLStmt::bindTime(int idx, const time_t &value)
{
    ...
    //return 0;
    return bindString(idx, lunar::Time2Str(value));
}
...
{
    return lunar::TypeUtil::Atoi(m_cur[idx]);
}
...
{
    return lunar::TypeUtil::Atof(m_cur[idx]);
}
...
time_t MySQLRes::getTime(int idx)
{
    ...
    return lunar::Str2Time(m_cur[idx]);
}
...
ISQLData::ptr MySQL::query(const char *format, va_list ap)
{
    ...
    m_hasError = false;
    ISQLData::ptr rt(new MySQLRes(res,
...
int MySQLTransaction::execute(const std::string &sql)
{
    if (m_isFinished)
    {
        LUNAR_LOG_ERROR(g_logger) << "transaction is finished, sql=" << sql;
        return -1;
    }
...
MySQLTransaction::MySQLTransaction(MySQL::ptr mysql, bool auto_commit)
    : m_mysql(mysql), m_autoCommit(auto_commit), m_isFinished(false),
      m_hasError(false)
{
}
    ...
    if (it != m_conns.end())
    {
        if (!it->second.empty())
        {
            ...
            if (!rt->isNeedCheck())
            {
                ...
                return MySQL::ptr(rt,
                                  std::bind(&MySQLManager::freeMySQL,
                                            this,
                                            name,
                                            std::placeholders::_1));
            }
            ...
            {
                LUNAR_LOG_WARN(g_logger) << "reconnect " << name << " fail";
                return nullptr;
            }
        }
    }
                          ...
                          va_list ap)
{
    ...
    if (!conn)
    {
        LUNAR_LOG_ERROR(g_logger) << "MySQLManager::execute, get(" << name
                                  << ") fail, format=" << format;
        return -1;
    }
                                  ...
                                  va_list ap)
{
    ...
    if (!conn)
    {
        LUNAR_LOG_ERROR(g_logger) << "MySQLManager::query, get(" << name
                                  << ") fail, format=" << format;
        return nullptr;
    }
