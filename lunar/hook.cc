...
//     lunar::Config::Lookup("tcp.connect.timeout", 5000, "tcp connect timeout");
...
#undef XX
    ...
    unsigned int sleep(unsigned int seconds)
    {
        ...
        (lunar::Scheduler::*): 这表示这是一个指向 lunar::Scheduler 类中成员函数的指针。
        (lunar::Fiber::ptr, int thread): 这是成员函数的参数列表。
    */
        iom->addTimer(seconds * 1000,
                      std::bind((void(lunar::Scheduler::*)(lunar::Fiber::ptr,
                                                           int thread)) &
                                    lunar::IOManager::schedule,
                                iom,
                                fiber,
                                -1));
        lunar::Fiber::YieldToHold();
    ...
    {
        return connect_with_timeout(sockfd,
                                    addr,
                                    addrlen,
                                    lunar::s_connect_timeout);
    }
                   ...
                   socklen_t optlen)
    {
        ...
        if (level == SOL_SOCKET)
        {
            ...
            {
                lunar::FdCtx::ptr ctx =
                    lunar::FdMgr::GetInstance()->get(sockfd);
                if (ctx)
                {
                    const timeval *v = (const timeval *)optval;
                    ctx->setTimeout(optname,
