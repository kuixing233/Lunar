...
Socket::ptr Socket::CreateTCP(lunar::Address::ptr address)
{
    Socket::ptr sock(new Socket(address->getFamily(), TCP, 0));
    return sock;
}

Socket::ptr Socket::CreateUDP(lunar::Address::ptr address)
{
...
Socket::ptr Socket::accept()
{
    ...
    if (newsock == -1)
    {
        LUNAR_LOG_ERROR(g_logger) << "accept(" << m_sock << ") errno=" << errno
                                  << " errstr=" << strerror(errno);
        return nullptr;
    }
...
bool Socket::bind(const Address::ptr addr)
{
    ...
    if (uaddr)
    {
        ...
        {
            lunar::FSUtil::Unlink(uaddr->getPath(), true);
        }
    }
...
bool Socket::reconnect(uint64_t timeout_ms)
{
    if (!m_remoteAddress)
    {
        LUNAR_LOG_ERROR(g_logger) << "reconnect m_remoteAddress is null";
        return false;
    }
...
bool Socket::connect(const Address::ptr addr, uint64_t timeout_ms)
{
    ...
    {
        if (::connect_with_timeout(m_sock,
                                   addr->getAddr(),
                                   addr->getAddrLen(),
                                   timeout_ms))
        {
...
bool Socket::listen(int backlog)
{
    if (!isValid())
    {
        LUNAR_LOG_ERROR(g_logger) << "listen error sock=-1";
        return false;
    }
...
Address::ptr Socket::getRemoteAddress()
{
    ...
    {
        //LUNAR_LOG_ERROR(g_logger) << "getpeername error sock=" << m_sock
                                 ...
                                 const std::string &key_file)
{
    ...
    if (SSL_CTX_use_PrivateKey_file(m_ctx.get(),
                                    key_file.c_str(),
                                    SSL_FILETYPE_PEM) != 1)
    {
        LUNAR_LOG_ERROR(g_logger)
...
SSLSocket::ptr SSLSocket::CreateTCP(lunar::Address::ptr address)
{
