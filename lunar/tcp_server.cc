...
static lunar::ConfigVar<uint64_t>::ptr g_tcp_server_read_timeout =
    lunar::Config::Lookup("tcp_server.read_timeout",
                          (uint64_t)(60 * 1000 * 2),
...
TcpServer::TcpServer(lunar::IOManager *worker,
    ...
    : m_worker(worker), m_ioWorker(io_worker), m_acceptWorker(accept_worker),
      ...
      m_name("lunar/1.0.0"), m_isStop(true)
{
}
                     ...
                     bool ssl)
{
    ...
    for (auto &addr : addrs)
    {
        ...
        if (!sock->listen())
        {
            LUNAR_LOG_ERROR(g_logger) << "listen fail errno=" << errno
...
{
    LUNAR_LOG_DEBUG(g_logger) << "TcpServer::startAccept start";
    while (!m_isStop)
    {
        ...
        if (client)
        {
            ...
            LUNAR_LOG_DEBUG(g_logger) << "TcpServer::startAccept handleClient";
...
{
    LUNAR_LOG_DEBUG(g_logger) << "TcpServer::start start";
    if (!m_isStop)
    {
        return true;
    }
    ...
    {
        LUNAR_LOG_DEBUG(g_logger) << "TcpServer::start startAccept_1";
        m_acceptWorker->schedule(
            std::bind(&TcpServer::startAccept, shared_from_this(), sock));
        LUNAR_LOG_DEBUG(g_logger) << "TcpServer::start startAccept_2";
    }
    LUNAR_LOG_DEBUG(g_logger) << "TcpServer::start stop";
    return true;
}
...
// static void Message(const lunar::Socket::ptr& conn) {
//     SocketStream ss(conn);
//     std::string message;
//     message.resize(128);
//     ss.read(&message[0], 128);

//     LUNAR_LOG_INFO(g_logger) << message;
