#ifndef __LUNAR_OBJECT_POOL_H__
#define __LUNAR_OBJECT_POOL_H__
...
public:
    ...
    ObjectPool(
        ...
        std::function<void(std::shared_ptr<T>)> resetter =
            [](std::shared_ptr<T> obj) {})
        : m_maxSize(maxSize), m_creator(creator), m_destroyer(destroyer),
          m_resetter(resetter)
    {
    }
        ...
        if (it != m_inUse.end() && it->second)
        {
            ...
            else
            {
                LUNAR_LOG_WARN(g_logger)
                    << "Warning: Pool is full. Discarding object.";
            }
        }
        else
        {
            LUNAR_LOG_WARN(g_logger)
                << "Warning: Object not released properly.";
        }
