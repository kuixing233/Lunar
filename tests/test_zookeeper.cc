...
void on_watcher(int type, int stat, const std::string& path, lunar::ZKClient::ptr client) {
    ALPHA_LOG_INFO(g_logger) << " type=" << type
        << " stat=" << stat
        << " path=" << path
        << " client=" << client
        << " fiber=" << lunar::Fiber::GetThis()
    ...
    if(stat == ZOO_CONNECTED_STATE) {
        if(g_argc == 1) {
            ...
            if(rt == ZOK) {
                ALPHA_LOG_INFO(g_logger) << "[" << lunar::Join(vals.begin(), vals.end(), ",") << "]";
            } else {
