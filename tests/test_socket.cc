...
static lunar::Logger::ptr g_looger = ALPHA_LOG_ROOT();
...
void test_socket() {
    ...
    //    }
    //}
    lunar::IPAddress::ptr addr = lunar::Address::LookupAnyIPAddress("www.bilibili.com");
...
void test2() {
    lunar::IPAddress::ptr addr = lunar::Address::LookupAnyIPAddress("www.baidu.com:80");
    if(addr) {
        ALPHA_LOG_INFO(g_looger) << "get address: " << addr->toString();
    } else {
        ALPHA_LOG_ERROR(g_looger) << "get address fail";
...
void test_server() {
    ...
    lunar::IOManager::GetThis()->addEvent(client_sock->getSocket(), lunar::IOManager::READ, [client_sock](){
