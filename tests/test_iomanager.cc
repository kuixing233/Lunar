...
void test_fiber() {
    ...
    if(!connect(sock, (const sockaddr*)&addr, sizeof(addr))) {
        ...
        lunar::IOManager::GetThis()->addEvent(sock, lunar::IOManager::READ, [](){
            ALPHA_LOG_INFO(g_logger) << "read callback";
        });
        lunar::IOManager::GetThis()->addEvent(sock, lunar::IOManager::WRITE, [](){
            ALPHA_LOG_INFO(g_logger) << "write callback";
            //close(sock);
            lunar::IOManager::GetThis()->cancelEvent(sock, lunar::IOManager::READ);
            close(sock);
        });
    } else {
