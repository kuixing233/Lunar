#include "../lunar/fiber.h"
#include "../lunar/iomanager.h"
#include "../lunar/log.h"
#include "../lunar/scheduler.h"
#include "../lunar/thread.h"
#include "../lunar/timer.h"
#include <arpa/inet.h>
#include <fcntl.h>
#include <iostream>
#include <string.h>
#include <sys/epoll.h>
#include <sys/socket.h>
#include <sys/types.h>
#include <unistd.h>

lunar::Logger::ptr g_logger = LUNAR_LOG_ROOT();

int sock = 0;

void test_fiber()
{
    LUNAR_LOG_INFO(g_logger) << "test_fiber sock=" << sock;

    //sleep(3);

    //close(sock);
    //lunar::IOManager::GetThis()->cancelAll(sock);

    sock = socket(AF_INET, SOCK_STREAM, 0);
    fcntl(sock, F_SETFL, O_NONBLOCK);

    sockaddr_in addr;
    memset(&addr, 0, sizeof(addr));
    addr.sin_family = AF_INET;
    addr.sin_port = htons(80);
    inet_pton(AF_INET, "115.239.210.27", &addr.sin_addr.s_addr);

    if (!connect(sock, (const sockaddr *)&addr, sizeof(addr)))
    {
    }
    else if (errno == EINPROGRESS)
    {
        LUNAR_LOG_INFO(g_logger)
            << "add event errno=" << errno << " " << strerror(errno);
        lunar::IOManager::GetThis()->addEvent(
            sock,
            lunar::IOManager::READ,
            []() { LUNAR_LOG_INFO(g_logger) << "read callback"; });
        lunar::IOManager::GetThis()->addEvent(
            sock,
            lunar::IOManager::WRITE,
            []() {
                LUNAR_LOG_INFO(g_logger) << "write callback";
                //close(sock);
                lunar::IOManager::GetThis()->cancelEvent(
                    sock,
                    lunar::IOManager::READ);
                close(sock);
            });
    }
    else
    {
        LUNAR_LOG_INFO(g_logger) << "else " << errno << " " << strerror(errno);
    }
}

void test1()
{
    std::cout << "EPOLLIN=" << EPOLLIN << " EPOLLOUT=" << EPOLLOUT << std::endl;
    lunar::IOManager iom(2, false);
    iom.schedule(&test_fiber);
}

lunar::Timer::ptr s_timer;
void test_timer()
{
    lunar::IOManager iom(1);
    // LUNAR_LOG_INFO(g_logger) << "+++++++++++++++++++++++";
    s_timer = iom.addTimer(
        1000,
        []() {
            static int i = 0;
            LUNAR_LOG_INFO(g_logger) << "hello timer i = " << i;
            if (++i == 3)
            {
                s_timer->reset(2000, true);
            }
            if (i == 6)
            {
                s_timer->cancel();
            }
        },
        true);
}

int main(int argc, char **argv)
{
    // test1();
    test_timer();
    return 0;
}